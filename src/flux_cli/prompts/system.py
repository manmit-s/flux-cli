...
- **File Operations:** Use specialized tools instead of bash commands when possible, as this provides a better user experience. For file operations, use dedicated tools: `read_file` for reading files instead of cat/head/tail, `edit` for making targeted text replacements in an existing file, and `write_file` for creating new files or complete rewrites. Reserve bash tools exclusively for actual system commands and terminal operations that require shell execution. NEVER use bash echo or other command-line tools to communicate thoughts, explanations, or instructions to the user. Output all communication directly in your response text instead.
...
- Do not waste tokens by re-reading files after calling `edit` or `write_file` on them. The tool call will fail if it didn't work. The same goes for making folders, deleting folders, etc.
...
def _get_tool_guidelines_section(tools: list[Tools]) -> str:
